...
CASE 
    WHEN etl.STATUS = NULL AND (SELECT DESCRIPTION FROM USR_UA_DESIGNATIONSTATUSCODE WHERE ID=dle.DESIGNATIONSTATUSCODEID) = 'Not Assigned' THEN 'No Change'
	WHEN etl.STEWARDSHIP_NAME = NULL AND LEN(dle.STEWARDSHIPNAME) = 0 THEN 'No Change' 
    WHEN etl.PREFERRED_SCHOOL = NULL AND (SELECT DESCRIPTION FROM USR_UA_DESIGNATIONPREFERREDSCHOOLCODE WHERE ID=dle.PREFERREDSCHOOLCODEID) = 'Not Assigned' THEN 'No Change'
	WHEN etl.ESTABLISHED_BY = NULL AND LEN(dle.ESTABLISHEDBY) = 0 THEN 'No Change' 
	WHEN etl.CONSTITUENT_TYPE = NULL AND LEN(dle.CONSTITUENTTYPE) = 0 THEN 'No Change' 
	WHEN etl.FUND_RESTRICTION = NULL AND LEN(dle.FUNDRESTRICTION) = 0 THEN 'No Change'
	WHEN etl.MOA_DATE = dle.MOADATE THEN 'No Change'
	WHEN etl.RESOLUTION_DATE = dle.RESOLUTIONDATE THEN 'No Change'
	ELSE 'Changed' END as ChangeFlag
FROM #ETL AS etl
